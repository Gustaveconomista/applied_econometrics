...
##### Data generating process function (equation 7) - EXPONENTIAL DISTRIBUTION #####
generate_data = function(G, NG) {
  # CHANGED: Using shifted exponential distribution to have mean=0, var=1
  # For exponential with rate λ: mean = 1/λ, var = 1/λ²
  # To get mean=0, var=1: use (Exp(1) - 1) which has mean=0, var=1
  # Or more generally: a*(Exp(λ) - 1/λ) + shift to get desired moments
  
  # Generate exponential with rate=1 and shift to mean=0, var=1
  zg = rexp(G, rate = 1) - 1           # z_g ~ Exp(1) - 1 (mean=0, var=1)
  epsilon_g = rexp(G, rate = 1) - 1    # ε_g ~ Exp(1) - 1 (mean=0, var=1)
  
  data = data.frame()
  for (g in 1:G) {
    zig = rexp(NG, rate = 1) - 1         # z_ig ~ Exp(1) - 1 (mean=0, var=1)
    epsilon_ig = rexp(NG, rate = 1) - 1  # ε_ig ~ Exp(1) - 1 (mean=0, var=1)
...
##### Main simulation loop for G = 5, 10 - EXPONENTIAL DISTRIBUTION CASE #####
cat("=== HOMOSKEDASTIC CASE WITH EXPONENTIAL DISTRIBUTION ===\n")
cat("DGP: y_ig = 0 + 1*x_ig + u_ig\n")
cat("     x_ig = z_g + z_ig, u_ig = ε_g + ε_ig\n")
cat("     z_g, z_ig, ε_g, ε_ig ~ Exp(1) - 1 (mean=0, var=1, right-skewed)\n")
cat("Cluster sizes:", paste(cluster_sizes, collapse = ", "), "\n\n")
...
for (G in cluster_sizes) {
  ...
  for (r in 1:R) {
    ...
    data = generate_data(G, NG)  # USING EXPONENTIAL DGP
    model = lm(y ~ x, data = data)
    beta_hat = coef(model)[2]
...
##### Display results for each cluster size #####
for (G in cluster_sizes) {
  cat("\n--- EXPONENTIAL Distribution Results for G =", G, "clusters ---\n")
  print(results_list[[as.character(G)]], digits = 3)
}

# Compare expected patterns
cat("\n=== EXPECTED IMPACT OF EXPONENTIAL vs NORMAL DISTRIBUTION ===\n")
cat("Key differences with exponential distribution:\n")
cat("- RIGHT-SKEWED: Exponential has skewness = 2 vs Normal = 0\n")
cat("- ASYMMETRIC TAILS: Different tail behavior affects extreme values\n")
cat("- BOUNDED BELOW: Exponential bounded at 0 (after shift), Normal unbounded\n\n")

cat("Expected effects on inference methods:\n")
cat("- Bootstrap methods: Should remain robust (distribution-free property)\n")
cat("- Asymptotic methods: May show sensitivity to skewness and tail behavior\n")
cat("- Wild bootstrap: Particularly designed to handle asymmetric distributions\n")
cat("- BCA bootstrap: Should provide bias correction for skewed distributions\n\n")

cat("Theoretical predictions:\n")
cat("- Over-rejection patterns similar to Table 2 but potentially magnified\n")
cat("- Bootstrap-t methods should maintain good performance\n")
cat("- Asymptotic refinement still expected from bootstrap-t procedures\n")
cat("- Moulton-type may show different performance due to skewness\n")

# Save results
saveRDS(results_list, file = "results_list_exp.rds")
